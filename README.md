...
1. **一次只发一条消息**：概览与正文全部打包进**同一条**合并转发聊天记录，
   避免几十条气泡刷屏。
2. **标题 → 子转发**：MC 百科正文里 `common-text-title` 的每一个标题是一个节点，
   标题与下一个标题之间的段落、列表项、图片按原始顺序**挂在它下面**，
...
记录内的结构恒为「记录 → 标题节点 → 内容节点」三层，正好是 QQ 支持的最大嵌套深度：
所有标题互为兄弟节点（层级由编号 1 / 1.1 / 1.1.1 体现），内容挂在各自标题下。
只有顶层节点数超过 `max_nodes_per_message` 时才会拆成多条记录，
且不会拆开任何一个标题的子节点。
